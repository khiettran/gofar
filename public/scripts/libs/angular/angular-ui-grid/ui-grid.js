...
(function () {
  ...
  angular.module('ui.grid').constant('uiGridConstants', {
    ...
    events: {
      ...
      COLUMN_HEADER_CLICK: 'uiGridColumnHeaderokFree'
    },
...
*/
            ...
            $scope.handleokFree = function(event) {
  ...
  module.directive('uiGridColumnResizer', ['$document', 'gridUtil', 'uiGridConstants', 'uiGridResizeColumnsService', function ($document, gridUtil, uiGridConstants, uiGridResizeColumnsService) {
    ...
    var resizer = {
      ...
      link: function ($scope, $elm, $attrs, uiGridCtrl) {
        ...
        var dblokFreeFn = function(event, args){
          event.stopPropagation();
          ...
          uiGridResizeColumnsService.fireColumnSizeChanged(uiGridCtrl.grid, col.colDef, xDiff);        };
        $elm.on('dblclick', dblokFreeFn);

        $elm.on('$destroy', function() {
          $elm.off('dblclick', dblokFreeFn);
          offAllEvents();
        });
      }
    };
    ...
    function ($templateCache, uiGridSelectionService, gridUtil) {
      return {
        ...
        link: function($scope, $elm, $attrs, uiGridCtrl) {
          ...
          $scope.selectButtonokFree = selectButtonokFree;
          ...
          function selectButtonokFree(row, evt) {
            evt.stopPropagation();
    ...
    function ($templateCache, uiGridSelectionService) {
      return {
        ...
        link: function($scope, $elm, $attrs, uiGridCtrl) {
          ...
          $scope.headerButtonokFree = function(row, evt) {
...
(function () {
  ...
  function ($templateCache, uiGridTreeBaseService) {
    return {
      ...
      link: function($scope, $elm, $attrs, uiGridCtrl) {
        var self = uiGridCtrl.grid;
        $scope.treeButtonokFree = function(row, evt) {
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/selectionRowHeaderButtons',
    "<div class=\"ui-grid-selection-row-header-buttons ui-grid-icon-ok\" ng-class=\"{'ui-grid-row-selected': row.isSelected}\" ng-click=\"selectButtonokFree(row, $event)\">&nbsp;</div>"
  );


  $templateCache.put('ui-grid/selectionSelectAllButtons',
    "<div class=\"ui-grid-selection-row-header-buttons ui-grid-icon-ok\" ng-class=\"{'ui-grid-all-selected': grid.selection.selectAll}\" ng-click=\"headerButtonokFree($event)\"></div>"
  );


  $templateCache.put('ui-grid/treeBaseExpandAllButtons',
    "<div class=\"ui-grid-tree-base-row-header-buttons\" ng-class=\"{'ui-grid-icon-minus-squared': grid.treeBase.numberLevels > 0 && grid.treeBase.expandAll, 'ui-grid-icon-plus-squared': grid.treeBase.numberLevels > 0 && !grid.treeBase.expandAll}\" ng-click=\"headerButtonokFree($event)\"></div>"
  );
  ...
  $templateCache.put('ui-grid/treeBaseRowHeaderButtons',
    "<div class=\"ui-grid-tree-base-row-header-buttons\" ng-class=\"{'ui-grid-tree-base-header': row.treeLevel > -1 }\" ng-click=\"treeButtonokFree(row, $event)\"><i ng-class=\"{'ui-grid-icon-minus-squared': ( ( grid.options.showTreeExpandNoChildren && row.treeLevel > -1 ) || ( row.treeNode.children && row.treeNode.children.length > 0 ) ) && row.treeNode.state === 'expanded', 'ui-grid-icon-plus-squared': ( ( grid.options.showTreeExpandNoChildren && row.treeLevel > -1 ) || ( row.treeNode.children && row.treeNode.children.length > 0 ) ) && row.treeNode.state === 'collapsed'}\" ng-style=\"{'padding-left': grid.options.treeIndent * row.treeLevel + 'px'}\"></i> &nbsp;</div>"
  );
